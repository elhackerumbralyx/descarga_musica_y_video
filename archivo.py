...
# Función para convertir el archivo descargado a MP3 usando ffmpeg
def convertir_ffmpeg(ruta_archivo):
    nombre_base, _ = os.path.splitext(ruta_archivo)
    archivo_destino = f"{nombre_base}.mp3"
    try:
        # Ejecuta el comando de ffmpeg para convertir el archivo a mp3
        subprocess.run(["ffmpeg", "-i", ruta_archivo, "-vn", "-ar", "44100", "-ac", "2", "-b:a", "192k", archivo_destino], check=True)
        messagebox.showinfo("Conversión Completa", f"El archivo se ha convertido a MP3 en {archivo_destino}.")
    except Exception as e:
        messagebox.showerror("Error en la Conversión", f"No se pudo convertir el archivo a MP3: {str(e)}")
...
def descargar_video():
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            ruta_archivo = ydl.prepare_filename(info)
